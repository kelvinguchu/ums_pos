...
export default function DailyReportsPage() {
  ...
  const getDateRange = (option: string): DateRange => {
    ...
    switch (option) {
      ...
      default:
        return { startDate: new Date(), endDate: new Date(), label: "Today's Report" };
    }
  };
  ...
  const handleYesterdayReport = async () => {
    try {
      ...
      const metrics = {
        totalSales: yesterdaySales.reduce((sum, sale) => sum + sale.total_price, 0),
        averageDailySales: yesterdaySales.reduce((sum, sale) => sum + sale.total_price, 0),
        totalMeters: yesterdaySales.reduce((sum, sale) => sum + sale.batch_amount, 0),
        metersByType: yesterdaySales.reduce((acc: { [key: string]: number }, sale) => {
  ...
  const handleTimeRangeReport = async (option: string) => {
    ...
    try {
      ...
      const sales = await getSaleBatches();
      const filteredSales = filterSalesByDateRange(sales, dateRange.startDate, dateRange.endDate);
      const metrics = calculateReportMetrics(filteredSales, dateRange.startDate, dateRange.endDate);
      ...
      handleReportDownload(blob, dateRange.label.toLowerCase().replace(/\s+/g, '-'));
    } catch (error) {
      handleReportError(error);
    } finally {
      setIsGeneratingOther(false);
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-end">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              {isGeneratingOther ? "Preparing..." : "Other Reports"} <ChevronDown className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
